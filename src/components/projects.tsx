...
import { Skill } from '@/lib/types'; // Assuming you have a types file, or define it here
...
const ProjectItem: React.FC<ProjectItemProps> = ({ title, company, dates, description, allSkills }) => {
    return (
...
interface ProjectsProps {
    projects: Omit<ProjectItemProps, 'allSkills'>[];
    allSkills: Skill[];
}
        ...
        <section className="order-2">
            ...
            <div className="mt-6">
                {projects.map((project, index) => (
                    <ProjectItem key={index} {...project} allSkills={allSkills} />
                ))}
            </div>
        </section>
    );
